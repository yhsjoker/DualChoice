...
- **响应参数说明**（除subject、name、idNumber、subSubjectOptions外其它字段均可以为空）:
...
- **URL**: `/api/graduateManagementSecretary/students/{college_id}`

- **请求方式**: `GET`

- **请求参数**:

  - `college_id` (string): 学院编号
...
- **URL**: `/api/graduateManagementSecretary/teachers/{college_id}`
...
- **URL**: `/api/graduateManagementSecretary/studentForm/{studentId}`
